...
public class AudioPlayer implements MediaPlayer.OnCompletionListener, MediaPlayer.OnBufferingUpdateListener,
            ...
            switch (msg.what) {
                case TIME_MSG:
                    ...
                    if (getStatus() == CustomMediaPlayer.Status.STARTED
                        ...
                        //UI类型处理事件
                        XAudio.getInstance().onAudioProgessEvent(new AudioProgressEvent(getStatus(), getCurrentPosition(), getDuration()));
    ...
    public void onCompletion(MediaPlayer mp) {
        //发送播放完成事件,逻辑类型事件
        XAudio.getInstance().onAudioCompleteEvent();
    }

    @Override
    public boolean onError(MediaPlayer mp, int what, int extra) {
        //发送当次播放实败事件,逻辑类型事件
        Log.e(TAG, "onError: AudioPlayer报错失败，what：" + what + " extra:" + extra);
        XAudio.getInstance().onAudioErrorEvent();
        return false;
    }
    ...
    private void start() {
        ...
        //发送start事件，UI类型处理事件
        XAudio.getInstance().onAudioStartEvent();
    }

    /**
     * 对外提供的加载音频的方法
     */
    public void load(BaseAudioBean audioBean) {
        if (mMediaPlayer == null){
            init();
        }
        try {
            ...
            //发送加载音频事件，UI类型处理事件
            XAudio.getInstance().onAudioLoadEvent(new AudioLoadEvent(audioBean));
        } catch (IOException e) {
            e.printStackTrace();
            XAudio.getInstance().onAudioErrorEvent();
        }
    ...
    public void pause() {
        if (getStatus() == CustomMediaPlayer.Status.STARTED) {
            ...
            //发送暂停事件,UI类型事件
            XAudio.getInstance().onAudioPauseEvent();
        }
    }
    ...
    public void release() {
        ...
        mHandler.removeCallbacksAndMessages(null);
        //发送销毁播放器事件,清除通知等
        XAudio.getInstance().onAudioReleaseEvent();
    }
